  ...
  return (
    <div 
      className={cn(
        ...
        color === 'gold' ? 'bg-gradient-to-br from-gravity-gold/20 to-yellow-900/20 border border-gravity-gold/30' : 
        color === 'silver' ? 'bg-gradient-to-br from-white/10 to-gray-700/10 border border-white/20' : 
    ...
    >
      <div className="absolute top-0 right-0 w-20 h-20 opacity-10">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <path d="M0,0 L100,0 L100,100 Z" fill={
            color === 'gold' ? '#F9B72B' : 
            color === 'silver' ? '#C0C0C0' : 
            '#CD7F32'
          } />
      ...
      <div className={cn(
        ...
        color === 'gold' ? 'bg-gravity-gold/20 text-gravity-gold' :
        color === 'silver' ? 'bg-white/10 text-white' :
      ...
      <h3 className={cn(
        ...
        color === 'gold' ? 'text-gravity-gold' :
        color === 'silver' ? 'text-white' :
        'text-amber-700'
      )}>{title}</h3>
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] pointer-events-none opacity-5 z-0">
        <div className="w-full h-full animate-rotate">
          <svg viewBox="0 0 100 100" width="100%" height="100%">
            <polygon points="50,10 90,90 10,90" stroke="#F9B72B" strokeWidth="0.5" fill="#F9B72B" fillOpacity="0.3" />
          ...
          {[
            {
              ...
              amount: "$500",
              description: "For the project that thinks outside the box with a truly unique approach."
            },
            {
              icon: <Award size={22} />,
              title: "Best UI/UX",
              amount: "$500",
              description: "For the most visually stunning and user-friendly project."
            },
            {
              icon: <Trophy size={22} />,
              title: "Best Rookie Team",
              amount: "$500",
              description: "For the most impressive project by first-time hackathon participants."
            },
            {
              icon: <Medal size={22} />,
              title: "Mystery Choice",
              amount: "???",
              description: "A special prize with mysterious criteria, revealed during the event."
            }
          ].map((prize, index) => (
            <PrizeCard
              key={prize.title}
